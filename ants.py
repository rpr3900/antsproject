...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chewing > 0:
            ...
            self.eat_bee(bee_selector(self.place.bees))
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        def already_queen_on_board(gamestate):
            place = self.place.entrance
            while place != None:
                if place.ant != None and place.ant.is_queen():
                    return True
                else:
        ...
        if already_queen_on_board(gamestate):
            self.impostor = True
            ...
            place = self.place.exit
            while place != None:
                if place.ant == None or place.ant.alreadyBuffed:
                    place = place.exit
                else:
                    place.ant.buff()
                    place = place.exit
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        self.health -= amount
        if self.health <= 0 and self.impostor == False:
            bees_win()
        else:
    ...
    def remove_from(self, place):
        if place.ant is self and place.ant.impostor:
            place.ant = None
        elif place.ant is None:
            assert False, '{0} is not in {1}'.format(self, place)

#self.health is instance attribute
#reduce health, need to call the super method for reducing health. it decrements health and it also removes it 
#from the gameboard
#484 bees_win shouldnt be set to a location, call bees_win properly
#only signal bees_win if QueenAnt if real QueenAnt
